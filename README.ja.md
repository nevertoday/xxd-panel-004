...
# 🦁 XXD Panel 004｜都市一本線手帳

### 信頼できる現実の場面から、収集したくなる現代線描ポスターへ
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 004 作例 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 004 作例 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 004 作例 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 004 作例 8"></td>
  </tr>
</table>

## 3:4 上下構成作例

16:9 組とは別の独立した4点で、3:4 上下の完成キャンバスを生成。上部は現実の写真を保ち、下部は Panel 004 の原文プロンプトに従って再構成しています。英語コピーは各写真の内容・空気・含意だけから生成し、全点を個別に確認しました。

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 004 追加上下作例 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 004 追加上下作例 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 004 追加上下作例 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 004 追加上下作例 4"></td>
  </tr>
</table>
...
> 土地の現実 · 精密な単線 · 幾何学的遠近 · 主題色 · 都市ブランド文字

XXD Panel 004 は、Codex と互換 Agent のための画像生成 Skill です。テーマ、場所、住所、座標、参考画像から、まず日常の温度を持つ一つの信頼できる現実場面を作り、その空、街路、看板、交通、店先、人の状態、または主題構造を、細く正確な線と明快な遠近法の現代ポスターへ抽出します。
...
**Panel 004** は、原写真の核を保ちながら、次の視覚方向を目指す制作に向いています：**信頼できる現実の場面から、収集したくなる現代線描ポスターへ**。まず用途と期待する結果を確認し、そこから構図、サイズ、文字、納品形式を決めます。
...
> XXD Panel 004 でこの画像を処理して、まず最適な構図とサイズを提案して。

視覚方向を先に確認したい場合は、上の作例をご覧ください。準備ができたら[使い始める](#使い始める)へ進めます。全モードと引数は折りたたみ資料から必要に応じて確認できます。
<!-- xxd-human-intro:end -->

<!-- xxd-panel-benefit:start -->
## すぐに判断：XXD Panel 004 はあなたに合う？

| 知りたいこと | このスタイルの答え |
|---|---|
| **得られる完成作品** | 信頼できる現実の場面から、収集したくなる現代線描ポスターへ |
| **ひと目で分かる特徴** | 土地の現実 · 精密な単線 · 幾何学的遠近 · 主題色 · 都市ブランド文字 |
...
Claude Code では同じフォルダを `~/.claude/skills/xxd-panel-004` にリンクできます。インストール後に Agent セッションを再起動してください。
...
テーマだけでも参考画像付きでも呼び出せます。モード、参考のないテーマに必要な画角、文字設定を確認し、壁紙では連動／独立と端末サイズも確認します。

詳細仕様：

- [Skill ワークフロー](SKILL.md)
- [中国語ランタイムアダプター](references/xxd-panel-004-prompt.zh-CN.md)
- [英語ランタイムアダプター](references/xxd-panel-004-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 004 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-004 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-004 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-004 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-004 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 004 を適用し、共通設定は一度だけ解決
/xxd-panel-004 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
- 選択した通常モードごとに1点を返し、`wallpaper-pack` を選ぶと独立壁紙4点を追加します。`全部` は入力テーマ1件につき7点を同じ新規タスクフォルダ直下へ出力し、一覧コラージュやモード別サブフォルダは作りません。
