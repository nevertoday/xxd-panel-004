...
[![Four Modes](https://img.shields.io/badge/Modes-4-d75d32?style=flat-square)](#four-outputs-one-reality-plus-line-logic)
...
<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-05.png" alt="XXD Panel 004 Sample 5"></td>
    <td width="50%"><img src="./assets/examples/sample-06.png" alt="XXD Panel 004 Sample 6"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-07.png" alt="XXD Panel 004 Sample 7"></td>
    <td width="50%"><img src="./assets/examples/sample-08.png" alt="XXD Panel 004 Sample 8"></td>
  </tr>
</table>

## Additional 3:4 Top–Bottom Samples

These four works reuse the same four independent sources as the 16:9 set above, now regenerated as complete 3:4 top–bottom canvases. The reality photograph stays above; the lower design follows Panel 004's original brief. English copy is generated only from each photograph's content, mood, or implication, and every result was individually reviewed.

<table>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-09.png" alt="XXD Panel 004 additional top-bottom sample 1"></td>
    <td width="50%"><img src="./assets/examples/sample-10.png" alt="XXD Panel 004 additional top-bottom sample 2"></td>
  </tr>
  <tr>
    <td width="50%"><img src="./assets/examples/sample-11.png" alt="XXD Panel 004 additional top-bottom sample 3"></td>
    <td width="50%"><img src="./assets/examples/sample-12.png" alt="XXD Panel 004 additional top-bottom sample 4"></td>
  </tr>
</table>
...
**Panel 004** is suited to work that must retain the source's essential content while pursuing this visual direction: **Build one credible reality scene, then distil it into a collectible modern line poster**. It begins with intended use and desired outcome; layout, size, text, and delivery format follow from those needs.
...
> Use XXD Panel 004 on this image and recommend the best layout and size first.
...
004 reverses that logic:

```text
confirm theme and reliable facts → establish one unified reality anchor → lock at least three local or thematic cues → distil with precise line and geometric perspective → derive colour from climate, light, material, and culture → integrate title and place information into one grid
```
...
- **Precise line translation:** distil with fine single line, clear perspective, overlap, and measured abstraction rather than tracing or applying icons.
- **Ordered detail rhythm:** use density, spacing, overlap, whitespace, and sparse local weight instead of realistic shading.
...
/xxd-panel-004 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB
...
/xxd-panel-004 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP
...
/xxd-panel-004 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-004 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-004 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 004 to a whole directory; shared settings resolve once
/xxd-panel-004 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB
...
Claude Code users may link the same directory to `~/.claude/skills/xxd-panel-004`. Restart the agent session after installation.

```text
$xxd-panel-004
Theme: an ordinary rainy morning around Kyoto's Demachiyanagi. Make top-bottom and design-only; use natural Japanese copy.
```

You may invoke the Skill with only a theme or with reference images. It asks for modes, a canvas when a theme has no reference, and copy settings; wallpaper mode also asks for linked/independent continuity and device sizes.

Full specifications:

- [Skill workflow](SKILL.md)
- [Chinese runtime adapter](references/xxd-panel-004-prompt.zh-CN.md)
- [English runtime adapter](references/xxd-panel-004-prompt.en.md)
...
**See the place truthfully first; preserve it with the fewest necessary lines.**
