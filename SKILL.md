---
name: xxd-panel-004
description: "Create XXD Panel 004 raster artwork from one image or a directory batch while treating the bundled original style brief as the sole aesthetic authority. Supports one or more top-bottom, left-right, design-only, and four-device wallpaper outputs; multiple ratios or exact sizes; prompt-generated, user-exact, or text-free typography; and inline parameters. Use when the user invokes xxd-panel-004 or requests the exact Panel 004 style."
---

# XXD Panel 004

Create fresh PNG artwork with `references/004-source.md` as the **sole creative and aesthetic authority**. This Skill is a runtime shell: it resolves delivery variables, appends them to the untouched source-brief body, executes bitmap generation, and verifies the result. It must not write a second art direction.

## Authority boundary

1. Read `references/004-source.md` completely immediately before building every generation request.
...
## Input rule for Panel 004

Panel 004 may start from a theme, place, street, address, coordinates, or a reference image. Treat explicit factual material as the reality anchor. When a real place is named and current facts matter, verify them through an available browsing/research route before prompting; do not invent location features. The source brief still controls all visual decisions.
...
```text
/xxd-panel-004 <source-or-directory> --mode top-bottom,design-only \
...
1. 上下对照（原图在上，004 设计在下）
2. 左右对照（原图在左，004 设计在右）
...
```text
[VERBATIM SOURCE-BRIEF BODY FROM references/004-source.md,
...
```text
~/Desktop/xxd/xxd-panel-004/<fresh-task>/
...
- the result follows `references/004-source.md`, especially its own colour, material, composition, whitespace, and typography requirements, without an outer Skill palette or added art direction;
...
- `references/xxd-panel-004-prompt.zh-CN.md`
- `references/xxd-panel-004-prompt.en.md`

These files document the same minimal adapter. They never replace `references/004-source.md`.
